...
export const ProjectParallax = ({ project }: { project: Project }) => {
  ...
  return (
    <div className="h-[70dvh] pt-40 overflow-hidden antialiased relative flex flex-col self-auto [perspective:2000px] [transform-style:preserve-3d] z-10">
      ...
      >
        <motion.div className="flex flex-row-reverse space-x-reverse space-x-20 mb-20">
          {firstRow.map((image, index) => (
            <ImageCard url={image.url} key={image.url + index} />
          ))}
        </motion.div>
...
export const Header = ({ project }: { project: Project }) => {
  return (
    <div className="max-w-7xl relative ml-auto py-20 md:py-40 px-4 w-full text-right">
      ...
      <p className="text-sm md:text-xl mb-5">{project.description}</p>
