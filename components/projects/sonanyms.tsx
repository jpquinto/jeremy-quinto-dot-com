...
export const Sonanyms = () => {
  ...
  const description1 = "a basic multi-player synonym guessing game built with next.js and aws. players can go head to head to test their vocabulary skills by trying to guess synonyms for a given word. the game features **real-time multiplayer** functionality using aws websocket API Gateway and DynamoDB. i implemented **user authentication** using clerk so users can save their progress and level up their account."

  const description2 = "i architected a pipeline to generate synonym data using the Datamuse API and store it in a dynamodb table, as well as a **real-time matchmaking system** that manages queues that players join. there is also a simple admin dashboard where i can add new words and synonyms to the database. coming soon!"
